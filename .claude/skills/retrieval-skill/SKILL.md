...
# 🏎️ Retrieval Skill - Meru Semantic Layer (v3.0)
...
> 💡 **Purpose**: The Retrieval Skill provides the interface to the Meru semantic layer, allowing for instant discovery across millions of knowledge chunks. It bridges the gap between raw data storage and active intelligence.
...
Use this skill when:
- ✅ Asked to "search the vault" or "find info on X".
- ✅ Building a **Source Lattice** for the Content Engine.
- ✅ Performing **Retrieval-Augmented Classification** (Stage 3 Analysis).
- ✅ Looking for **cross-domain connections** (e.g., matching Enneagram to Bioelectronics).
- ✅ verifying if a concept already exists before adding a new note.
...
→ Result: Top papers from `03-Resources/Technology/` with direct snippets.
...
→ Result: Chunks from `02-Areas/Consciousness-Models/` showing consistent pattern development.
