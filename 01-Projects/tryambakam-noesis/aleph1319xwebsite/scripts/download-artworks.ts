/**
 * Downloads baroque paintings from the Art Institute of Chicago API.
 * Creates a manifest.json used by the infinite canvas.
 *
 * Usage: npx tsx scripts/download-artworks.ts
 */
...
async function fetchAllArtworks(): Promise<ArticArtwork[]> {
  ...
  while (allArtworks.length < 250) {
    ...
    const valid = data.data.filter(
      (a: ArticArtwork) => a.image_id && a.thumbnail
    );
...
async function main() {
  ...
  for (let i = 0; i < artworks.length; i++) {
    ...
    if (fs.existsSync(filepath)) {
      console.log(
        `[${i + 1}/${artworks.length}] Skipping (exists): ${artwork.title.slice(0, 40)}`
      );
      manifest.push(item);
      continue;
    }
    ...
    console.log(
      `[${i + 1}/${artworks.length}] Downloading: ${artwork.title.slice(0, 40)}`
    );
